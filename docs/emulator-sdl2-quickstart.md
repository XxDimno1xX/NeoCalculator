...
## Build vs. Run vs. Debug (and VS Code)

Four different things people confuse — especially in VS Code, where “Build”, the
**Run/Debug** play button, and the PlatformIO debug configs all look like ways to
“start” NumOS. They are **not** interchangeable:

| I want to… | Command | Opens the emulator window? | Needs an ESP32 board? |
|:--|:--|:--:|:--:|
| **Build the firmware** (image for real hardware) | `pio run -e esp32s3_n16r8` | No | No (just compiles) |
| **Build the emulator** (compile only) | `pio run -e emulator_pc` | **No** | No |
| **Run the emulator** (build **and** launch) | `pio run -e emulator_pc -t exec` | **Yes** | No |
| **Debug ESP32 hardware** (JTAG) | VS Code *Run/Debug* → “PIO Debug” | No | **Yes** |

The single most common macOS/Linux snag is doing the **build** step and expecting a
window: `pio run -e emulator_pc` **only compiles**. To actually see NumOS, you must
**run** it — add the **`-t exec`** target (PlatformIO builds if needed, then launches
the native binary):
...
`-t exec` is the most portable run command because PlatformIO launches the binary
from **its own** build directory — you never have to know the path (see
[Where the build goes](#where-the-build-goes-the-cpiobuild-path)). Equivalent
convenience wrappers:

| OS | One-step run |
|:--|:--|
| **macOS / Linux** | `./scripts/run-emulator.sh` (wraps `pio run -e emulator_pc -t exec`) |
| **Linux** (advanced: forwards `--frames`/`--headless`/`--script`…) | build, then `./scripts/run-emulator-linux.sh <flags>` |
| **Windows** | `./scripts/run-emulator-windows.ps1` (also resolves `SDL2.dll`) |

> `-t exec` cannot forward arguments to the emulator program. For the Phase 3A+
> flags (`--frames`, `--headless`, `--script`, `--screenshot`, …) build first, then
> run the binary directly via the platform run script (above), which forwards
> everything after it.

### In VS Code — what to click (and what to avoid)

- ✅ **Run the emulator:** **Terminal → Run Task… → “Run NumOS Emulator”**
  (defined in [`.vscode/tasks.json`](../.vscode/tasks.json); it runs
  `pio run -e emulator_pc -t exec`). Or just type that command in any terminal.
- ❌ **Do not press Run/Debug (F5)** for the emulator. The repo's debug configs are
  **ESP32-only**: every entry in [`.vscode/launch.json`](../.vscode/launch.json) is a
  `platformio-debug` config bound to `esp32s3_n16r8` and `firmware.elf`. Launching
  one tries to attach a **JTAG debugger to physical hardware** and fails on a
  machine with no board connected. (That file is auto-generated by PlatformIO, which
  is why it has no emulator entry — use the **task** above instead.)
- ❌ **Do not pick “Electron Main.”** That's a generic VS Code/Node debug template,
  not part of NumOS — NumOS is a **C++/SDL2** desktop program, not an Electron app,
  so “Electron Main” has nothing to attach to.
- ❌ **“PIO Debug”, “PIO Debug (skip Pre-Debug)”, “PIO Debug (without uploading)”** —
  all three are the ESP32 JTAG configs above; none run the desktop emulator.
...
the Windows firmware build working, and PlatformIO has **no per-environment
`build_dir`**, so it can't be scoped to just the emulator. So it stays — and is
already neutralized everywhere it matters:

- **CI** overrides it with the `PLATFORMIO_BUILD_DIR=.pio/build` job env (see
  [`.github/workflows/emulator-build.yml`](../.github/workflows/emulator-build.yml)),
  so no `C:` folder is ever created on the Linux runners.
- **You** have two clean options on macOS/Linux:
  1. **Use `-t exec`** (or `./scripts/run-emulator.sh`) and ignore the path entirely —
     PlatformIO runs the binary from wherever it built it.
  2. **Override the build dir** so everything lands in a normal `.pio/build`:
     ```bash
     PLATFORMIO_BUILD_DIR=.pio/build pio run -e emulator_pc -t exec
     ```
     With that set, `./scripts/run-emulator-linux.sh` finds the binary too.
...
# macOS (Homebrew) — install the PlatformIO CLI first if `pio` is "command not found"
brew install platformio
brew install sdl2 pkg-config
pio --version                   # confirm the CLI is on your PATH before building
pio run -e emulator_pc -t exec  # build AND open the window (plain `pio run` only compiles)
...
> **New to PlatformIO on a Mac?** Start here. This is the exact flow for a fresh
> machine (Apple Silicon M1/M2/M3 or Intel) where the very first thing you see is
> `zsh: command not found: pio`.

### `zsh: command not found: pio` — what it means

This is the #1 first-run snag on macOS, and it does **not** mean anything is broken.
It just means the PlatformIO **command-line tool** (`pio`) isn't installed — or isn't
on the PATH of *this* terminal — yet. Nothing about NumOS or your Mac is wrong; you
only need to install the CLI.

> **The VS Code PlatformIO IDE extension is *not* the `pio` CLI.** If you installed
> the PlatformIO extension in VS Code, PlatformIO lives **inside the extension** and
> is only available in the dedicated terminal VS Code opens for it — the alien-head
> icon in the sidebar → *PlatformIO Core CLI* (a.k.a. "Open PlatformIO Core CLI").
> A plain macOS **Terminal** (or iTerm) window does **not** inherit that — a normal
> terminal needs PlatformIO installed as an ordinary shell command. Installing it
> with Homebrew puts `pio` on your system PATH so it works in *any* terminal, VS Code
> or not.

### Prerequisites
- **[Homebrew](https://brew.sh)** — the macOS package manager.
- **PlatformIO Core CLI** — `brew install platformio` (puts `pio` on your PATH).
- **SDL2 + pkg-config** — `brew install sdl2 pkg-config`. Homebrew's SDL2 ships
  `pkg-config`/`sdl2-config`, so the **same discovery path as Linux** applies (see
  [How SDL2 is discovered](#how-sdl2-is-discovered-no-hardcoded-paths)); there are no
  hardcoded paths to set.

### Build & run (copy-paste)

```bash
brew install platformio          # PlatformIO CLI  (skip if you already have it)
brew install sdl2 pkg-config     # SDL2 desktop dependency + pkg-config for discovery
pio --version                    # sanity check — prints e.g. "PlatformIO Core, version 6.x"
pio run -e emulator_pc -t exec   # build AND open the emulator window (one step)
```

**Always run `pio --version` before `pio run -e emulator_pc`.** If it prints a
version, the CLI is on your PATH and the build will find it. If it *still* says
`command not found` after `brew install platformio`:

- **Close and reopen the Terminal** so a new shell picks up the updated PATH, then
  retry `pio --version`.
- On **Apple Silicon**, Homebrew installs to `/opt/homebrew/bin`; make sure that
  directory is on your PATH (a fresh `brew install` normally adds it). You can always
  invoke the CLI by its full Homebrew path: `$(brew --prefix)/bin/pio --version`.

> **Build vs. run.** Plain `pio run -e emulator_pc` **only compiles** — it does not
> open a window. The **`-t exec`** target (above) builds *and* launches, which is
> what you want when you "built it but nothing happened." See
> [Build vs. Run vs. Debug](#build-vs-run-vs-debug-and-vs-code). The convenience
> wrapper `./scripts/run-emulator.sh` does the same thing.

Once `pio --version` works, the line above builds the emulator exactly as on
Windows/Linux and opens the SDL2 window. Prefer `-t exec` (or `./scripts/run-emulator.sh`)
over `./scripts/run-emulator-linux.sh` on a fresh checkout: the helper script looks in
`.pio/build`, but the default `build_dir` puts the macOS binary in a literal `./C:/`
folder (see [Where the build goes](#where-the-build-goes-the-cpiobuild-path)), so the
helper won't find it unless you also set `PLATFORMIO_BUILD_DIR=.pio/build`.

> **Status:** the macOS path uses the same `pkg-config sdl2` discovery as Linux and is
> *expected to work*, but — like Linux — has not yet been proven by a green CI run on
> the author's machine. Treat it as "expected to work, pending first confirmation."
