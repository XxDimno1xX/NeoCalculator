...
#ifdef TICE
void mpz_mul_si(mpz_t & c,mpz_t & a,int B){
  bool neg=B<0;
  if (neg) B=-B;
  mp_int b;
  mp_init_set_int(&b,B);
...
namespace giac {
  ...
  use system malloc, heap size 90k  0x88050000 to 0x88067000
  allocsmall in system Python area 0x88067000 to 0x8807ffff, 
  we start at 0x88070000 
  alternative would be to use kmalloc, must change ../libc/stdlib.c
  add corresponding gint files
...
// format is currently ignored, keep only 6 digits (round) 
  ...
  void sprintfdouble(char * ch,const char * format,double d){
    ...
    if (i==d)
      ...
      sprintf(ch,"%f",d);    // %g does not work
    //dbg_printf("sprintf %s %g ",format,d);
    //dbg_printf(format,d);
    //dbg_printf("\n",format);
    // sprintf(ch,format,d);
#else
...
#define debugtypeptr _CPLXptr
    ...
    * ((ulonglong * ) this)=0;
    ...
    if (val==i && val!=1<<23){
      ...
      * ((ulonglong * ) this) = ulonglong(new ref_mpz_t(64)) << 16;
      ...
      if (signe)
	i=-i;
      unsigned int i1=sizeof(long)==4?0:i>>24;
      unsigned int i2=(unsigned int)i;
      mpz_set_ui(*_ZINTptr,i1);
      mpz_mul_2exp(*_ZINTptr,*_ZINTptr,24);
    ...
    * ((ulonglong * ) this)=0;
    ...
    if (val==i && val!=1<<23){
      ...
      * ((ulonglong * ) this) = ulonglong(new ref_mpz_t(64)) << 16;
      ...
      if (signe)
        i=-i;
      unsigned long long l1=i>>24;
      unsigned int i0=l1>>24;
      unsigned int i1=(unsigned int)(l1 & 0xffffff);
      unsigned int i2=(unsigned int)(i & 0xffffff);
      //dbg_printf("gen(longlong) i0=%i i1=%i i2=%i\n",i0,i1,i2);
      mpz_set_ui(*_ZINTptr,i0);
      mpz_mul_2exp(*_ZINTptr,*_ZINTptr,24);
  ...
  ref_vecteur * new_ref_vecteur(const vecteur & v){
    ...
    ptr->ref_count=1; // create an empty vector
    *(unsigned *)&ptr->v=ustl_vecteur_prolog;
    *((unsigned *)&ptr->v+1)=0;
    *((unsigned *)&ptr->v+2)=0;
    *((unsigned *)&ptr->v+3)=0;
    ptr->v=v;
...
#endif
    const gen * begin_ptr = _VECTptr->empty() ? nullptr : &(*_VECTptr)[0];
    if ( v.size()>1 &&
	 ( (((size_t) _VECTptr) &stack_mask) > stackptr ||
	   (((size_t) begin_ptr) &stack_mask) > stackptr )
	 )
      ctrl_c=interrupted=true;
    type=_VECT;
  ...
  ref_symbolic * new_ref_symbolic(const symbolic & s){
    ...
    ptr->s.feuille=s.feuille;
    memcpy(&ptr->s.sommet,&s.sommet,sizeof(s.sommet));
    return ptr;
  }
#endif

  gen::gen(const symbolic & s){
      // dbg_printf("build gen from symbolic 1 %s\n",s.print(context0).c_str());
#ifdef COMPILE_FOR_STABILITY
    control_c();
#endif
#ifdef SMARTPTR64
...
#endif
    //dbg_printf("build gen from symbolic 2 %s\n",_SYMBptr->print(context0).c_str());
    type = _SYMB;
    subtype = 0;
    //dbg_printf("build gen from symbolic 3\n");
    //dbg_printf("%s\n",print(context0).c_str());
    //dbg_printf("build gen from symbolic 4\n");
    if ( _SYMBptr->sommet!=at_restart && _SYMBptr->sommet!=at_purge && (((size_t) _SYMBptr) &stack_mask) > stackptr){
      dbg_printf("interrupt stack\n");
      ctrl_c=interrupted=true;
    }
  }
  ...
  gen::gen(const fraction & p){
    //dbg_printf("gen(fraction) %s/%s\n",p.num.print(context0).c_str(),p.den.print(context0).c_str());
    subtype=0;
  ...
  giac_float gen::FLOAT_val() const { 
    ...
    return _FLOAT_val;
#else
    longlong r = * (longlong *)(this) ; // TICE FIXME?
    // * (unsigned char *) (&r) = 0;
#ifdef BCD
    return * (giac_float *)(&r);     
#else
  ...
  gen & gen::operator = (const gen & a){
	...
	return *this;
      ...
      //dbg_printf("operator = \n");
      //dbg_printf("operator = source %s\n",a.print(context0).c_str());
...
#else
    static gen m=gen(longlong(1)<<61); // TICE FIXME
#endif
  ...
  static bool in_eval_vect(const gen & g,gen & evaled,int level,GIAC_CONTEXT){
    ...
    if (g._VECTptr->size()==1 && g._VECTptr->front().is_interval())
...
//      if (storcl_38(res,0,g0._IDNTptr->id_name,undef,false,contextptr)) return evalf2double_nock(res,level-1,contextptr);
	...
	*it=evalf2double_nock(*it,level,contextptr);
	...
	return f;
      ...
      if (f.type!=_DOUBLE_ && (f.type<_IDNT || f.type==_FRAC))
	f=evalf2double_nock(f,1,contextptr);
      return f;
    }
    if (g0.type==_CPLX){
  ...
  gen gen::conj(GIAC_CONTEXT) const {
	...
	if (_SYMBptr->feuille.type==_VECT && _SYMBptr->feuille._VECTptr->size()==2){
      ...
      if ( equalposcomp(analytic_sommets,_SYMBptr->sommet) || _SYMBptr->sommet==at_surd
#ifndef XLIGHT
           || _SYMBptr->sommet==at_erf
#endif
           )
  ...
  gen sym_add(const gen & a,const gen & b,GIAC_CONTEXT){
    ...
    if ( (a.is_interval() || adeuxpoints)&& a._SYMBptr->feuille.type==_VECT && a._SYMBptr->feuille._VECTptr->size()==2)
	...
	*logptr(contextptr) << "Warning constant+function with constant dependant of mute variable" << endl;
    ...
    if (b.is_neg() && a==b._SYMBptr->feuille)
	...
	return (*a._FRACptr)+b;
      if (b.is_prod() && b._SYMBptr->feuille.type==_VECT){
	const vecteur & bf=*b._SYMBptr->feuille._VECTptr;
	if (bf.size()==2 && is_integer(bf[0]) && bf[1].is_inv() && is_cinteger(bf[1]._SYMBptr->feuille))
	...
	return a+(*b._FRACptr);
      if (a.is_prod() && a._SYMBptr->feuille.type==_VECT){
	const vecteur & af=*a._SYMBptr->feuille._VECTptr;
	if (af.size()==2 && is_integer(af[0]) && af[1].is_inv() && is_cinteger(af[1]._SYMBptr->feuille))
  ...
  *((double *) &c) -= (*((double *) &a)) * (*((double *) &b));
...
#ifdef TICE
	mpz_mul_si(e->z,*b._ZINTptr,a.val);
#else      
      if (a.val<0){
...
#ifdef TICE
      mpz_mul_si(e->z,*a._ZINTptr,b.val);
#else
      if (b.val<0){
  ...
  gen pow(const gen & base,const gen & exponent,GIAC_CONTEXT){
	...
	if (exponent.type!=_INT_ || !res.is_exp())
	  return res;
      }
	...
	return base;
      if (exponent.is_prod() && exponent._SYMBptr->feuille.type==_VECT && exponent._SYMBptr->feuille._VECTptr->size()==2){
	gen e1=exponent._SYMBptr->feuille._VECTptr->front();
	gen e2=exponent._SYMBptr->feuille._VECTptr->back();
	if (e1.is_symb_of_sommet(at_ln) && e2.is_inv() && e2._SYMBptr->feuille.is_symb_of_sommet(at_ln) && base==e2._SYMBptr->feuille._SYMBptr->feuille)
  ...
  gen sym_mult(const gen & a,const gen & b,GIAC_CONTEXT){
	...
	*logptr(contextptr) << "Warning constant*function with constant dependant of mute variable" << endl;
    ...
    if (a.type<=_CPLX && b.is_inv()&& b._SYMBptr->feuille.type<=_CPLX)
      return fraction(a,b._SYMBptr->feuille).normal();
    if (b.type<=_CPLX && a.is_inv()&& a._SYMBptr->feuille.type<=_CPLX)
  ...
  */
...
#ifdef TICE
      mpz_mul_si(e->z,e->z,base);
#else
      mpz_mul_ui(e->z,e->z,base);
#endif
    }
    return e;
  }
  ...
  static gen rdivsimp(const gen & a,const gen & b){
    if (is_positive(-b,context0)) // ok
      return rdivsimp(-a,-b);
    //dbg_printf("rdivsimp %s %s\n",a.print(context0).c_str(),b.print(context0).c_str());
    gen c(gcd(a,b,context0));
    //dbg_printf("rdivsimp gcd %s\n",c.print(context0).c_str());
    if (c.type==_CPLX)
  ...
  */
  ...
  gen gen::operator () (const gen & i,const gen & progname,const context * contextptr) const{
    bool isprog=type==_FUNC || this->is_program() || this->is_symb_of_sommet(*at_program);
    if (!isprog){
      if (i.is_equal())
	...
	*logptr(contextptr) << gettext("Warning, evaluating (") << *this << ")(" << i << ") as a function not as a product" << endl;
    ...
    if (i.type==_DOUBLE_ && int(i._DOUBLE_val)==i._DOUBLE_val )
  ...
  bool gen::is_plus() const{
    return type==_SYMB && _SYMBptr->sommet==at_plus;
  }
  
  bool gen::is_pow() const{
    return type==_SYMB && _SYMBptr->sommet==at_pow;
  }
  
  bool gen::is_neg() const{
    return type==_SYMB && _SYMBptr->sommet==at_neg;
  }
  
  bool gen::is_inv() const{
    return type==_SYMB && _SYMBptr->sommet==at_inv;
  }
  
  bool gen::is_prod() const{
    return type==_SYMB && _SYMBptr->sommet==at_prod;
  }
  
  bool gen::is_interval() const{
    return type==_SYMB && _SYMBptr->sommet==at_interval;
  }
  
  bool gen::is_equal() const{
    return type==_SYMB && _SYMBptr->sommet==at_equal;
  }

  bool gen::is_exp() const{
    return type==_SYMB && _SYMBptr->sommet==at_exp;
  }
  
  bool gen::is_tan() const{
    return type==_SYMB && _SYMBptr->sommet==at_tan;
  }
    
  bool gen::is_program() const{
    return type==_SYMB && _SYMBptr->sommet==at_program;
  }
  ...
  static vecteur terme2unitaire(const gen & x,bool sorted,GIAC_CONTEXT){
	...
	if (vtmp[i].is_inv() && vtmp[i]._SYMBptr->feuille.is_pow()){
	...
	if (it->type<=_CPLX)
	  ...
	  if (it->is_inv() && it->_SYMBptr->feuille.type<=_CPLX)
  ...
  */
  ...
  gen liste2symbolique(const vecteur & v){
    ...
    for (;it!=itend;++it){
      ...
      if (is_zero(coeff))
	continue;
      if (tmp.is_prod() && tmp._SYMBptr->feuille.type==_VECT && tmp._SYMBptr->feuille._VECTptr->size()==1){
  ...
  */
  ...
  static gen _CPLXgcd(const gen & a,const gen & b){ // a & b must be gen
	...
	default:
      ...
      r=acopy%bcopy;
      //dbg_printf("CPLXgcd loop %s \% %s -> %s\n",acopy.print(context0).c_str(),bcopy.print(context0).c_str(),r.print(context0).c_str());
      acopy=bcopy;
      bcopy=r;
    }
  }
  ...
  void egcd(int a,int b,int &u,int &v,int &d){
    int u0=1,u1=0,u2,v0=0,v1=1,v2,q,r;
    for (;b;){
      q=a/b;
      u2=u0-q*u1;
      v2=v0-q*v1;
  ...
  void egcd(const gen &ac,const gen &bc, gen & u,gen &v,gen &d ){
    ...
    case _INT___INT_:
      u=v=d=0;
      egcd(a.val,b.val,u.val,v.val,d.val);
      break;
...
#if 1
    mpz_set_ui(e->z,1);
    for (unsigned long int k=2;k<=i;++k){
#ifdef TICE
      mpz_mul_si(e->z,e->z,k);
#else
      mpz_mul_ui(e->z,e->z,k);
#endif
    }
#else // does not work on tice
    mpz_fac_ui(e->z,i);
#endif
    return e;
  }
  ...
  static int try_parse(const string & s_orig,GIAC_CONTEXT){
	...
	i_sqrt_minus1(0,contextptr);
      dbg_printf("parse begin\n");
      res=giac_yyparse(scanner);
      dbg_printf("parse end\n");
      if (1){
        gen pg=parsed_gen(contextptr);
        dbg_printf("taille %i type %i\n",taille(pg,256),pg.type);
        dbg_printf("%s\n",pg.print(contextptr).c_str());
      }
  ...
  static gen aplatir_plus(const gen & g){
    ...
    if (g.is_plus() && g._SYMBptr->feuille.type==_VECT){
  ...
  static gen aplatir_plus_only(const gen & g){
	...
	*jt=aplatir_plus_only(*it);
	...
	if (it->type!=_SYMB || it->_SYMBptr->sommet!=at_plus){
	  v.push_back(*it);
	  break;
	}
	...
	it=f._VECTptr->begin();
      ...
      vreverse(v.begin(),v.end());
      return change_subtype(gen(symb_plus(v)),g.subtype);
    }
  ...
  static int protected_giac_yyparse(const string & chaine,gen & parse_result,GIAC_CONTEXT){
    ...
    if (g.type<=_FLOAT_){
      parse_result=g;//aplatir_plus_only(g); // FIXME aplatir_fois_plus
  ...
  */
  ...
  string print_DOUBLE_(double d,GIAC_CONTEXT){
    ...
    *u &= 0xe0;
    ...
    if (sf==2){
      // engineering format
      int ndigits=int(std::floor(std::log10(d)+0.5));
      ndigits = 3*(ndigits /3);
  ...
  string gen::print(GIAC_CONTEXT) const{
    ...
    case _SYMB: 
      {
        //dbg_printf("_SYMB print subtype %i ptr %x\n",subtype,(size_t) _SYMBptr);
	int d=depth(*this,0,print_max_depth);
	if (d>=print_max_depth)
	  return "Too many embeddings";
        //dbg_printf("_SYMB depth %i\n",d);
      }
