...
namespace giac {
...
#endif
      if (complexmode && (lidnt(evalf(makevecteur(two_a,minus_b,delta),1,contextptr)).empty() || lvar(delta).size()==1) && is_positive(-delta,contextptr))
	delta=cst_i*sqrt(-delta,contextptr);
      else
	delta=sqrt(delta,contextptr);
      //delta=normalize_sqrt(delta,contextptr,false); // no abs in sqrt
  ...
  gen rationalize(const gen & g,const gen & x,GIAC_CONTEXT){
    ...
    if (s==1)
      return expr;
    // solve(sin(3x)=cos(x))
    if (expr.is_plus() && expr._SYMBptr->feuille.type==_VECT && expr._SYMBptr->feuille._VECTptr->size()==2){
  ...
  if (!is_undef(e_exact) && !has_num_coeff(e_exact)){
	...
	return vecteur(1,tmp);
      int lvs=0;
      if (tmp.is_prod() && tmp._SYMBptr->feuille.type==_VECT){
    ...
    lv=lvarx(expr,x);
    int s=int(lv.size());
    if (s==1 && lv[0].is_tan() && expr.is_prod() && expr._SYMBptr->feuille.type==_VECT){
  ...
  static gen symb_solution(const gen & g,const gen & var,GIAC_CONTEXT){
    ...
    for (;it!=itend;++it,++jt){
      // if (*it!=*jt)
      if (jt->is_equal() && jt->_SYMBptr->feuille[0]==*it)
	res.push_back(*jt);
      else
  ...
  gen _solve_uncompressed(const gen & args,GIAC_CONTEXT){
	...
	if (is_linear_wrt(eq1,var1,a,b,contextptr) && is_linear_wrt(eq2,var1,c,d,contextptr) && lvarxwithinv(makevecteur(a,b,c,d),var2,contextptr).size()>1 && !is_zero(recursive_normal(a*d-b*c,contextptr))){
	  ...
	  if (res.type==_VECT && !res._VECTptr->empty()){
	    for (unsigned i=0;i<res._VECTptr->size();++i){
	      ...
	      gen v2val=v2.is_equal()?v2._SYMBptr->feuille._VECTptr->back():v2;
	...
	if (is_linear_wrt(eq1,var2,a,b,contextptr) && is_linear_wrt(eq2,var2,c,d,contextptr) && lvarxwithinv(makevecteur(a,b,c,d),var1,contextptr).size()>1 && !is_zero(recursive_normal(a*d-b*c,contextptr))){
	  ...
	  if (res.type==_VECT && !res._VECTptr->empty()){
	    for (unsigned i=0;i<res._VECTptr->size();++i){
	      ...
	      gen v1val=v1.is_equal()?v1._SYMBptr->feuille._VECTptr->back():v1;
	...
	if (a1.is_program() || a2.is_program())
	  return gensizeerr(contextptr);
	if (a2==0 && a1.is_plus() && a1._SYMBptr->feuille._VECTptr->size()==2){
  ...
  vecteur true_lidnt(const gen & g){
	...
	if (v1.is_equal() && v1._SYMBptr->feuille.type==_VECT && !v1._SYMBptr->feuille._VECTptr->empty())
  ...
  gen in_fsolve(vecteur & v,GIAC_CONTEXT){
    ...
    if (v[0].is_program())
      swapgen(v[0],v[1]);
    if (v[1].is_program()){ // scilab-like syntax
  ...
  //grad
    ...
    if (gguess.is_interval() && (s<4 || v[3].subtype!=_INT_PLOT)){
      int iszero=-1;
...
#define inplace_ppz ppz
  ...
  vecteur coeffs(const polynome & p,vector<index_m> rmonomials,environment * env){
    ...
    for (unsigned k=0;k<p.coord.size();++k){
      int pos=giac::find(rmonomials,p.coord[k].index);
  ...
  bool fglm_lex(const vectpoly & G,vectpoly & Glex,unsigned maxpow,environment * env,GIAC_CONTEXT){
	...
	CERR << CLOCK() << " reduce end " << "\n";
      ...
      for (unsigned i=0;i<R.coord.size();++i){
        pos=giac::find(rmonomials,R.coord[i].index);
	if (pos<0){
	...
	ligne.push_back(0);
      for (unsigned i=0;i<R.coord.size();++i){
        int pos=giac::find(rmonomials,R.coord[i].index);
...
#ifdef XLIGHT
    return  vecteur(1,gensizeerr("Not implemented"));
#else
  ...
  static gen greduce(const gen & g,const vecteur & l,const vectpoly & eqp,const gen & order,bool with_cocoa,GIAC_CONTEXT,vecteur * quo=0){
	...
	for (int i=0;i<int(quo->size());++i){
      ...
      vreverse(quo->begin(),quo->end()); // the gbasis was reversed
      return res;
    }
...
#if !defined CAS38_DISABLED && !defined FXCG && !defined TICE && !defined NSPIRE_NEWLIB
