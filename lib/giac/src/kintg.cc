...
namespace giac {
  ...
  gen extract_cst(gen & u,const gen & x,GIAC_CONTEXT){
    if (!u.is_prod() || u._SYMBptr->feuille.type!=_VECT)
      return 1;
  ...
  static gen substconj_(const gen & N,const gen & X,const gen & x,const gen & dephasage_,bool residue_only,GIAC_CONTEXT){
    //dbg_printf("substconj N=%s X=%s dephasage=%s residue=%i\n",N.print(context0).c_str(),X.print(context0).c_str(),dephasage_.print(context0).c_str(),residue_only);
  ...
  gen _surd(const gen & args,GIAC_CONTEXT){
    ...
    if (!has_evalf(a,aa,1,contextptr)){
      if (c.type==_INT_ && c==b && (c.val %2 ==0 || (a.is_pow() && a._SYMBptr->feuille[1].type==_INT_ && a._SYMBptr->feuille[1].val % c.val==0)) )
  ...
  static gen makelnatan(const gen & N,const gen & X,const gen & c0,int n,bool residue_only,GIAC_CONTEXT){
    //dbg_printf("mklnatan N=%s X=%s c0=%s n=%i residue=%i\n",N.print(context0).c_str(),X.print(context0).c_str(),c0.print(context0).c_str(),n,residue_only);
    gen c(c0),res(0);
    if (n%2){
  ...
  static bool integrate_deno_length_2(const polynome & num,const vecteur & v,const vecteur & l,const vecteur & lprime,gen & res,bool residue_only,int intmode,GIAC_CONTEXT){
    ...
    //dbg_printf("int deno2 %s\n",gen(v).print(contextptr).c_str());
    gen X=l.front();
    if (X.type==_VECT)
      X=X._VECTptr->front();
  ...
  static bool integrate_sqrt(gen & e,const gen & gen_x,const vecteur & rvar,gen & res,gen & remains_to_integrate,int intmode,GIAC_CONTEXT){ // x and a power
	...
	*/
	...
	if (n){
	  ...
	  vreverse(col0.begin(),col0.end()); // C at the beginning, Q at the end
  ...
  static gen integrate_rational_end(vector< pf<gen> >::iterator & it,vector< pf<gen> >::const_iterator & itend,const gen & x,const gen & xvar,const vecteur & l,const vecteur & lprime,const polynome & ipnum,const polynome & ipden,const gen & ratpart,gen & remains_to_integrate,int intmode,GIAC_CONTEXT){
	...
	if (uselog){
	  ...
	  if (tmpatan.is_tan())
	    ...
	    gen beta,gamma;
	    if (  //0 && 
		  xvar.is_tan() && is_linear_wrt(tmpatan,xvar,beta,gamma,contextptr) && is_strictly_greater(4*beta,gamma*gamma,contextptr) ){
  ...
  static bool integrate_step0(gen & e,const gen & gen_x,vecteur & l1,vecteur & m1,gen & res,gen & remains_to_integrate,GIAC_CONTEXT,int intmode){
    ...
    *logptr(contextptr) << gettext("! Integration of abs or sign assumes constant sign by intervals:\nCheck ") << l1 << "\n";
	...
	if (val2.is_symb_of_sommet(at_sin) || val2.is_tan())
  ...
  bool is_elementary(const vecteur & v,const gen & x){
    ...
    for (;it!=itend;++it){
      if (*it==x)
	continue;
      if (!it->is_exp() && (!it->is_symb_of_sommet(at_ln)) )
	return false;
    }
    return true;
  }
  ...
  gen integrate_id_rem(const gen & e_orig,const gen & gen_x,gen & remains_to_integrate,GIAC_CONTEXT,int intmode){
	...
	*logptr(contextptr) << "! assuming integration and sum commutes" << "\n";
    ...
    if (rvarsize<=TRY_FU_UPRIME){ // otherwise no hope
      ...
      ++it; // don't try x!
      for (;it!=itend;++it){
	if (it->is_symb_of_sommet(at_fsolve) || it->is_equal())
	  continue;
	gen df=derive(*it,gen_x,contextptr);
	gen tmprem;
	fu=ratnormal(rdiv(e,df,contextptr),contextptr);
        //dbg_printf("integrate f(u)u' *%s %s\n",it->print().c_str(),fu.print().c_str());
    ...
    *logptr(contextptr) << gettext("integrate step 2 ") << e << "\n";
	...
	fu=symb_prod(vv);
      ...
      bool recur=rvarsize>2 && gen_x.type==_IDNT && strcmp(gen_x._IDNTptr->id_name,"t_nostep")==0 && u.is_pow(); // workaround for some stupid integrals like integrate(sin((d*x + c)^(2/3)*b + a)/(f*x + E),x);
      if (!recur && 
    ...
    *logptr(contextptr) << gettext("integrate step 3 ") << e << "\n";
    ...
    //dbg_printf("integrate trig frac %s\n",e.print().c_str());
...
#ifdef NO_STDEXCEPT
    //dbg_printf("primitive %s\n",primitive.print().c_str());
    if (ordonne){
      gen xval=assumeeval(x,contextptr);
      ...
      //dbg_printf("limit borne_sup s\n",rs.print().c_str());
      gen ri=limit(primitive,*x._IDNTptr,borne_inf,1,contextptr);
      //dbg_printf("limit borne_inf %s\n",ri.print().c_str());
      res=rs-ri;
  ...
  bool tegral(const gen & f,const gen & x,const gen & a_,const gen &b_,const gen & eps,int nmax,gen & value,bool exactcheck,GIAC_CONTEXT){
    //dbg_printf("tegral f=%s a=%s b=%s eps=%s",f.print().c_str(),a_.print().c_str(),b_.print().c_str(),eps.print().c_str());
...
**********************************************************************/
  static vecteur solveP_x_plus_1_minus_P_x(const vecteur & Q){
      ...
      vecteur Q_copy(Q);
      vreverse(Q_copy.begin(),Q_copy.end());
      m.push_back(Q_copy);
      m=mtran(m);
  ...
  bool gosper(const polynome & P,const polynome & Q,const polynome & R,polynome & Y,gen & deno,GIAC_CONTEXT){
    ...
    vreverse(m.begin(),m.end()); // higher coeff at the beginning
    // last column is P
    lrdm(vP,p);
    m.push_back(vP);
    m=mtran(m);
  ...
  bool is_hypergeometric(const gen & e,const identificateur &x,vecteur &v,polynome & P,polynome & Q,polynome & R,GIAC_CONTEXT){
    ...
    gen a,b;
    for (int i=0;i<vs;++i){
      if (v[i].is_exp() && !is_linear_wrt(v[i]._SYMBptr->feuille,x,a,b,contextptr))
	return false;
    }
  ...
  gen seqprod(const gen & g,int type,GIAC_CONTEXT){
	...
	return _map(makesequence(v2,f),contextptr);
      ...
      if (v.size()==3 && v[1].is_equal() && v[1]._SYMBptr->feuille[1].is_interval()){
	...
	return seqprod(makevecteur(v[0],v1,v2,v3,v[2]),type,contextptr);
      ...
      if (v.size()==3 && !v[1].is_equal() && g.subtype==_SEQ__VECT)
  ...
  */ 
  ...
  gen odesolve(const gen & t0orig,const gen & t1orig,const gen & f,const gen & y0orig,double tstep,bool return_curve,double * ymin,double * ymax,int maxstep,GIAC_CONTEXT){
    //dbg_printf("odesolve t0=%s t1=%s f=%s y0=%s tstep=%f ymin=%f ymax=%f maxstep=%i\n",t0orig.print().c_str(),t1orig.print().c_str(),f.print().c_str(),y0orig.print().c_str(),tstep,ymin?*ymin:0,ymax?*ymax:0,maxstep);
	...
	if (return_curve && abs(t1_e,contextptr)._DOUBLE_val>1e40)
...
#if 1
    double y[dim]; // ALLOCA(double, y, dim*sizeof(double));// double * y =(double *)alloca(dim*sizeof(double));
#else
     ...
     */
    gen tolerance=epsilon(contextptr)>1e-5?epsilon(contextptr):1e-5;
	...
	for (int i=0;i<dim;i++){
      ...
      if (is_undef(hopt))
	break;
      //dbg_printf("nstep=%i t_e=%s y5=%s y4=%s tstep=%f hopt=%s err=%f\n",nstep,t_e.print().c_str(),gen(y_final5).print().c_str(),gen(y_final4).print().c_str(),tstep,hopt.print().c_str(),err);
  ...
  static gen odesolve(const vecteur & w,GIAC_CONTEXT){
    ...
    double tstep=0;
    if ( t0t.is_interval()){ // functional form
  ...
  gen fourier_an(const gen & f,const gen & x,const gen & T,const gen & n,const gen & a,GIAC_CONTEXT){
    ...
    primi=_integrate(makesequence(f*cos(omega*n*x,contextptr),x,a,ratnormal(a+T,contextptr)),contextptr);//_integrate(gen(makevecteur(f*cos(omega*n*x,contextptr),x,a,ratnormal(a+T,contextptr)),_SEQ__VECT),contextptr);
    gen an=iT*primi;
    if (n!=0) 
      an=2*an;
  ...
  gen fourier_bn(const gen & f,const gen & x,const gen & T,const gen & n,const gen & a,GIAC_CONTEXT){
    ...
    gen primi,iT=inv(T,contextptr);
    gen omega=ratnormal(2*cst_pi*iT,contextptr);
    primi=_integrate(makesequence(f*sin(omega*n*x,contextptr),x,a,ratnormal(a+T,contextptr)),contextptr);//_integrate(gen(makevecteur(f*sin(omega*n*x,contextptr),x,a,ratnormal(a+T,contextptr)),_SEQ__VECT),contextptr);
    gen an=2*iT*primi;
    return has_num_coeff(an)?an:recursive_normal(an,contextptr);
  }
  ...
  gen fourier_cn(const gen & f,const gen & x,const gen & T,const gen & n,const gen & a,GIAC_CONTEXT){
    ...
    gen primi,iT=inv(T,contextptr);
    gen omega=ratnormal(2*cst_pi*iT,contextptr);
    primi=_integrate(makesequence(f*exp(-cst_i*omega*n*x,contextptr),x,a,ratnormal(a+T,contextptr)),contextptr);//_integrate(gen(makevecteur(f*exp(-cst_i*omega*n*x,contextptr),x,a,ratnormal(a+T,contextptr)),_SEQ__VECT),contextptr);
    gen cn=iT*primi;
...
#if defined FXCG || !defined USE_GMP_REPLACEMENTS || defined TICE
  ...
  gen _periodic(const gen & g,GIAC_CONTEXT) {
	...
	return gentypeerr(contextptr);
      ...
      if ((x=fl[0]).type!=_IDNT || !fl[1].is_interval())
	return gentypeerr(contextptr);
