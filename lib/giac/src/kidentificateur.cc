...
#if defined GIAC_HAS_STO_38 || defined NSPIRE || defined NSPIRE_NEWLIB || defined FXCG || defined TICE
#if 0 // 38 mode
...
//  const gen & z__IDNT_e = * (gen *) & alias_z38;
...
#else
  static gen vx_var_storage(identificateur("x"));
  gen & vx_var(){
    return vx_var_storage;
  }
#endif
  ...
  */
  ...
  static gen vx_var_storage(x__IDNT_e);
  gen & vx_var(){
    return vx_var_storage;
  }
...
#endif
#if defined GIAC_HAS_STO_38 || defined NSPIRE_NEWLIB || defined FXCG || defined TICE
    for (;*c;++c){
      if (*c==' ')
	*c='_';
    }
  ...
  bool identificateur::in_eval(int level,const gen & orig,gen & evaled,const context * contextptr, bool No38Lookup) {
	...
	return false;
      ...
      if (it==cur->tabptr->end()){
#ifdef TICE
        if (strlen(id_name)==2 && (id_name[0]=='y' || id_name[0]=='Y') && (id_name[1]>='0' && id_name[1]<='9')){
          char buf[3]={0x5e,0,0};
          buf[1]=(id_name[1]=='0')?0x19:(0x10+(id_name[1]-'1'));
          string val=get_tivar(buf);
          if (val.size()){
            gen valg(val,contextptr);
            if (!valg.in_eval(level,evaled,contextptr->globalcontextptr))
              evaled=valg;
            return true;
          }
        }
