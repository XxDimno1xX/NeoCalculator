...
namespace giac {
  ...
  bool pcompose(const vecteur & v,const sparse_poly1 & p, sparse_poly1 & res,GIAC_CONTEXT){
    ...
    //dConsolePut(string("compose ")+gen(vcopy).print(contextptr)+string(" with ")+gen(pcopy).print(contextptr)+string("\n").c_str());
	...
	for (;sit!=sitend;++sit){
      ...
      // COUT << res << "\n";
      //dbg_printf("%s + %s * %s\n",gen(res).print().c_str(),it->print().c_str(),plcmn.print().c_str());
      if (!is_zero(*it))
  ...
  bool series__SPOL1(const gen & e_orig,const identificateur & x,const gen & lim_point,int ordre,int direction,sparse_poly1 & s,GIAC_CONTEXT){
	...
	if (
            ...
            0
#else
            temp__SYMB.sommet==at_Psi || temp__SYMB.sommet==at_Eta || temp__SYMB.sommet==at_Zeta
#endif
            ){
	...
	if (tf.type==_VECT && tf._VECTptr->size()==2){
      ...
      if (temp__SYMB.sommet==at_abs){
	if (!image_of_direction){
	  //*logptr(contextptr) << gettext("Sign error ") << s << "\n";
	...
	if (temp__SYMB.sommet==at_cos || temp__SYMB.sommet==at_sin){
      ...
      gen shift_coeff;
      if (!temp__SYMB.sommet.ptr()->series_expansion){
	*logptr(contextptr) << string(gettext("Not expandable "))+temp__SYMB.sommet.ptr()->s << "\n";
	return false;
      }
      int addorder=0;
      gen expansion;
      if (
          ...
          0
#else
          (temp__SYMB.sommet==at_Psi ||temp__SYMB.sommet==at_Eta ||temp__SYMB.sommet==at_Zeta) && temp__SYMB.feuille.type==_VECT
#endif
           ){
  ...
  */
  ...
  if (!indeterminate_prod && !contains(lidnt(first_try),unsigned_inf)){
	...
	bool hasnum=has_evalf(first_try,chknum,1,contextptr);
	first_try=recursive_ratnormal(first_try,contextptr);
	gen chk=first_try;//recursive_normal(first_try,contextptr);
	if (hasnum && !is_undef(chk) && abs(chk-chknum,contextptr)>1e-10 && abs(1-chk/chknum,contextptr)>1e-10){
	  chk=undef;
	  e=_simplify(e,contextptr);
	}
  ...
  *logptr(contextptr) << gettext("Warning, infinity is unsigned, perhaps you meant +infinity")<< "\n";
  ...
  gen _limit(const gen & args,GIAC_CONTEXT){
    ...
    gen e=v[1];
    if (s==2){
      if (calc_mode(contextptr)==1 && !v[1].is_equal())
  ...
  static gen sparse_poly12gen_expand(const sparse_poly1 & s,const identificateur & x,const gen & mrv_var,int ordre,gen & remains,bool with_order_size,GIAC_CONTEXT){
    ...
    if (mrv_fait.is_inv() && mrv_fait._SYMBptr->feuille.is_exp())
