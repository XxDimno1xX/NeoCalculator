...
#ifdef TICE
#include "k_csdk.h"
#include "main.h" // for freeze
#include "console.h" // draw_line
#endif
#include <stdexcept>
#include <cmath>
#include <cstdlib>
...
#else
#if !defined BESTA_OS && !defined FXCG && !defined TICE
#include <time.h>
#endif
#endif
...
namespace giac {
...
#if 0 // defined(GIAC_HAS_STO_38) && defined(VISUALC)
  ...
  void check_local_assign(const gen & g,const vecteur & prog_args,vecteur & res1,vecteur & res2,vecteur & res3,vecteur & res4,bool testequal,GIAC_CONTEXT){
    ...
    if (g.is_symb_of_sommet(at_sto)){
      gen &f=g._SYMBptr->feuille;
      if (f.type==_VECT && f._VECTptr->size()==2 && f._VECTptr->front().is_program()){
  ...
  static string printasprogram(const gen & feuille,const char * sommetstr,GIAC_CONTEXT){
    if ( (feuille.type!=_VECT) || (feuille._VECTptr->size()!=3) )
      return string(sommetstr)+('('+feuille.print(contextptr)+')');
    string res;
#if 1 // ndef XLIGHT // must keep in xcas mode for save/restore purpose
  ...
  gen symb_return2(const gen & arg,GIAC_CONTEXT){
    return gen(symbolic(at_return,arg));
  }
...
#pragma clang diagnostic pop
  ...
  static define_unary_function_eval2_index (86,__return,&symb_return2,_return_s,&printasreturn);
  ...
  static gen feuille_(const gen & g,const gen & interval,GIAC_CONTEXT){
    ...
    if (interval.is_interval()&& interval._SYMBptr->feuille.type==_VECT){
  ...
  gen _map(const gen & args,const context * contextptr){
    ...
    if (xcas_mode(contextptr)==1 || (to_map.type!=_FUNC && !to_map.is_program() && (objet.type==_FUNC || objet.is_program()))){
      objet=v[1];
      to_map=v.front();
    }
  ...
  gen _apply(const gen & args,const context * contextptr){
    ...
    if (to_apply.type!=_FUNC)
      n=n2-1;
    int nargs=1;
    if (to_apply.is_program() && to_apply._SYMBptr->feuille[0].type==_VECT)
  ...
  gen _dollar(const gen & args,GIAC_CONTEXT){
    ...
    if (b1.type==_VECT && b1.subtype==_SEQ__VECT && b1._VECTptr->size()==2){
      ...
      if (b.type==_VECT && b.subtype==_SEQ__VECT && b._VECTptr->size()==2 && b11.is_equal() && b._VECTptr->front().is_equal())
	b11=symb_equal(b._VECTptr->front()[1],b11[2]);
      return _dollar(makesequence(a,b11,b1._VECTptr->back()),contextptr);
    }
    if (a.is_interval() && a._SYMBptr->feuille.type==_VECT && a._SYMBptr->feuille._VECTptr->size()==2){
  ...
  void debug_loop(gen &res,GIAC_CONTEXT){
    ...
    w.push_back(dw);
    //Bdisp_AllClr_VRAM();
    os_fill_rect(0,STATUS_AREA_PX-2,LCD_WIDTH_PX,LCD_HEIGHT_PX+2-STATUS_AREA_PX,COLOR_WHITE);
	...
	m=giacmax(0,M-5);
      for (int i=m;i<=M;++i){
        os_draw_string_medium(dispx,dispy,COLOR_BLACK,(i==w[4].val?COLOR_SELECTED:COLOR_WHITE),(print_INT_(i)+":"+ws[i]).c_str(),0);
#ifdef TICE
        dispy+=14;
#else
        dispy+=6;
    ...
    draw_line(0,dispy+20 /* STATUS_AREA_PX*/,LCD_WIDTH_PX,dispy+20,COLOR_BLACK);
    dispx = 0;
    dispy += 4;
    // progs += "======\n";
    ...
    dispy=C58;
    PrintMini(dispx,dispy,(const Char *)"    next    |  step in    |  continue   |  kill   ",MINI_REV);      
    w.push_back(dw);
  ...
  gen _double_deux_points(const gen & args,GIAC_CONTEXT){
    ...
    if (b==at_revlist || b==at_reverse || b==at_sort || b==at_sorted || b==at_append || b==at_prepend || b==at_concat || b==at_extend || b==at_suppress || b==at_clear ){	
#if 1
    ...
    if (b.type==_SYMB){
      ...
      if (c==at_index)
	return _struct_dot(eval(args,1,contextptr),contextptr);
      if (c==at_revlist || c==at_reverse || c==at_sort || c==at_sorted || c==at_append || c==at_prepend || c==at_concat || c==at_extend || c==at_suppress || c==at_clear ){
#if 1
	return _struct_dot(args,contextptr);
#else
...
#endif
#ifndef RTOS_THREADX
#if !defined BESTA_OS && !defined NSPIRE && !defined FXCG && !defined TICE && !defined NSPIRE_NEWLIB
#ifdef HAVE_LIBPTHREAD
  ...
  */
  ...
  gen simplifier(const gen & g,GIAC_CONTEXT){
...
#if 0 // !defined FXCG && !defined TICE
	...
	if (wi.is_pow() && f.type==_VECT && f._VECTptr->size()==2 && f._VECTptr->back().type==_FRAC){
  ...
  bool no_context_sort_f(const gen &a,const gen &b){
    if (a==b) return false;
    if (qs_comp_f){
      gen c=(*qs_comp_f)(makesequence(a,b),qs_contextptr);
      if (c.type==_INT_)
  ...
  gen _sort(const gen & args,GIAC_CONTEXT){
    ...
    gen_sort_f(v.begin(),v.end(),no_context_sort_f);
#else
  ...
  gen convert_real(const gen & g,GIAC_CONTEXT){
    ...
    if (g.type==_REAL){
      return g;
      //if (dynamic_cast<real_interval *>(g._REALptr)) return _milieu(g,contextptr);
    }
    if (g.type==_CPLX)
...
#endif
#ifdef WITH_UNITS
    if (f.is_symb_of_sommet(at_unit)&&g.is_symb_of_sommet(at_unit)){
  ...
  gen _findhelp(const gen & g,GIAC_CONTEXT){
    ...
    const char * cmdptr=argss.c_str();
#if 0
    if (has_static_help(cmdptr,lang,howto,syntax,examples,related)){
#ifdef NSPIRE
...
#if defined RTOS_THREADX || defined NSPIRE_NEWLIB || defined FXCG || defined TICE
  ...
  */
  ...
  const mksa_unit __c_unit={2.99792458e8,1,0,-1,0,0,0,0};        
...
const mksa_unit __lambda0_unit={1.239841984e-6,1,0,0,0,0,0,0}; // inverse meter-electron volt relationship
  ...
  vecteur mksa_convert(const identificateur & g,GIAC_CONTEXT){
    ...
    if (pp.first!=pp.second && pp.second!=unitname_tab_end){
      //dbg_printf("call to mksa_register %s\n",unitptr_tab[pp.first-unitname_tab]);
    ...
    int nchar=1;
    if (it==itend && l>1){
      //dbg_printf("unit not found prefix=%c\n",s[0]);
      switch (s[0]){
      case 'Y':
	exposant=24;
	break;
    ...
    if (exposant!=0){
      ...
      //dbg_printf("exponent=%i new unit=%s\n",s.c_str());
    ...
    //dbg_printf("mksa_unit2vecteur %s",it->second);
  ...
  gen mksa_reduce(const gen & g,GIAC_CONTEXT){
    ...
    //dbg_printf("mksa g=%s v=%s\n",g.print().c_str(),gen(v).print().c_str());
  ...
  gen symb_unit(const gen & a,const gen & b,GIAC_CONTEXT){
    ...
    iterateur it=w.begin(),itend=w.end();
    for (;it!=itend;++it){
      string ss=it->print(contextptr);
      if (ss.size()>=6 && ss.substr(0,5)=="unit_")
        ss=ss.substr(5,ss.size()-5);
      if (ss[0]=='_')
        ss=ss.substr(1,ss.size()-1);
      //dbg_printf("make_unit ss=%s\n",ss.c_str());
      find_or_make_symbol("_"+ss,*it,0,false,contextptr);
    }
    //dbg_printf("make_unit b=%s v=%s w=%s\n",b.print().c_str(),gen(v).print().c_str(),gen(w).print().c_str());
  ...
  string printasunit(const gen & feuille_,const char * sommetstr,GIAC_CONTEXT){
    gen feuille=exactify_pow(feuille_);
    if (feuille.type!=_VECT || feuille._VECTptr->size()!=2)
  ...
  static gen unit(const gen & g,GIAC_CONTEXT){
    if (g.type!=_VECT || g._VECTptr->size()!=2)
      return gensizeerr(contextptr);
    return symb_unit(g._VECTptr->front(),g._VECTptr->back(),contextptr);
  }
  ...
  bool is_array_index(const gen & m,const gen & i,GIAC_CONTEXT){
	...
	gen g=(*i._VECTptr)[j];
	if (g.type==_IDNT || g.is_equal() || g.is_symb_of_sommet(at_deuxpoints) || g.is_symb_of_sommet(at_sto))
	  continue;
	return true;
      }
    }
  ...
  gen _struct_dot(const gen & g,GIAC_CONTEXT){
	...
	if (s.type==_FUNC)
      ...
      if (equalposcomp(plot_sommets,u) || u==at_clear
          //|| u==at_scatterplot || u==at_diagrammebatons
          )
    ...
    if ((a.type==_IDNT || a.type==_SYMB) && (u==at_revlist || u==at_reverse || u==at_sort || u==at_append || u==at_prepend || u==at_concat || u==at_extend || u==at_suppress || u==at_remove || u==at_insert ))
